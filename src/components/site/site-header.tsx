...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-ink/95 backdrop-blur-xl">
      ...
      {open ? (
        <div className="border-t border-white/10 bg-ink px-4 pb-6 lg:hidden">
          <nav aria-label="Mobile navigation" className="mx-auto max-w-7xl py-4">
            <div className="grid gap-2">
              {navItems.map((item) => (
                <Link
                  className="rounded-md px-3 py-3 text-base font-bold text-white transition hover:bg-white/10"
                  href={item.href}
                  key={item.href}
                  onClick={() => setOpen(false)}
                >
            ...
            <div className="mt-4 border-t border-white/10 pt-4">
              <p className="px-3 text-sm font-black text-gold">
                Services
              </p>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {services.map((service) => (
                  <Link
                    className="rounded-md border border-white/10 px-3 py-3 text-sm font-bold text-white hover:bg-white/5 transition"
                    href={`/services/${service.slug}`}
                    key={service.slug}
                    onClick={() => setOpen(false)}
                  >
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <Button asChild className="bg-gold text-ink hover:bg-gold/90">
                <Link href={calendlyUrl} onClick={() => setOpen(false)}>
                  <CalendarCheck className="h-4 w-4" />
                  Book Estimate
                </Link>
              </Button>
